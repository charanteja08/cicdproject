...
const SeedsSaplingsPage = () => {
  ...
  // Fetch crops once and derive the Seeds & Saplings slice client-side
  useEffect(() => {
    async function loadCrops() {
      try {
        const res = await fetch(`${API}/crops`);
        if (!res.ok) {
          console.error('Failed to fetch crops for Seeds & Saplings:', res.status, res.statusText);
          return;
        }
        const rows = await res.json();
        const mapped = rows.map((row) => mapCropToProduct(row, BACKEND_ORIGIN));
        const seedsOnly = filterBySection(mapped, 'seeds').map(product => ({
          ...product,
          category: product.cropType?.toLowerCase().includes('sapling') ? 'Saplings' : 'Seeds',
          type: product.cropType || 'Vegetable Seeds'
        }));
        setServerProducts(seedsOnly);
      } catch (err) {
  ...
  // Use only server products so admins fully control the catalog
            ...
            {hasProducts ? (
              ...
              >
                <Typography variant="h6" sx={{ fontFamily: "'Poppins', sans-serif", color: '#1B5E20', mb: 1 }}>
                  No seeds or saplings yet
                </Typography>
                <Typography variant="body2" sx={{ fontFamily: "'Inter', sans-serif", color: '#666' }}>
                  Add crops from the admin dashboard to start showcasing them here.
                </Typography>
              </Paper>
            )}
          </Grid>
